...
describe("node exec events", () => {
  ...
  it("enqueues exec.started events", async () => {
    ...
    expect(requestHeartbeatNowMock).toHaveBeenCalledWith({ reason: "exec-event:agent:main:main" });
  });
  ...
  it("enqueues exec.finished events with output", async () => {
    ...
    expect(requestHeartbeatNowMock).toHaveBeenCalledWith({ reason: "exec-event:node-node-2" });
  });
  ...
  it("enqueues exec.denied events with reason", async () => {
    ...
    expect(requestHeartbeatNowMock).toHaveBeenCalledWith({ reason: "exec-event:agent:demo:main" });
  });
});
